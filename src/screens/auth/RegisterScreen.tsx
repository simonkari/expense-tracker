...
export default function RegisterScreen() {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      console.log("Registration error handled in context")
    } finally {
    ...
    <SafeAreaView className={`flex-1 ${isDark ? "bg-gray-900" : "bg-white"}`}>
      <View className="flex-1 justify-center px-8">
        <View className="items-center mb-8">
          ...
          <Text className={`text-2xl font-bold mt-4 ${isDark ? "text-white" : "text-gray-800"}`}>Create Account</Text>
          <Text className={`text-sm mt-2 ${isDark ? "text-gray-300" : "text-gray-600"}`}>Sign up to get started</Text>
        </View>
        ...
        <View className="flex-row justify-center mt-8">
          <Text className={isDark ? "text-gray-300" : "text-gray-600"}>Already have an account? </Text>
          <TouchableOpacity onPress={() => navigation.navigate("Login" as never)} disabled={loading || localLoading}>
